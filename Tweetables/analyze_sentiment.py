...
# # Ensure nltk_data lives in a writable place on Windows
# NLTK_DIR = os.path.expanduser(r"~\AppData\Roaming\nltk_data")
# os.makedirs(NLTK_DIR, exist_ok=True)
# if NLTK_DIR not in nltk.data.path:
#     nltk.data.path.insert(0, NLTK_DIR)

# def _ensure_nltk_resources():
#     """Fetch tokenizer/tagger/corpora if missing. Supports old/new names."""
#     needed = [
#         ("tokenizers/punkt", "punkt"),
#         ("taggers/averaged_perceptron_tagger_eng", "averaged_perceptron_tagger_eng"),
#         ("taggers/averaged_perceptron_tagger", "averaged_perceptron_tagger"),  # fallback
#         ("corpora/stopwords", "stopwords"),
#         ("corpora/wordnet", "wordnet"),
#         ("corpora/omw-231.4", "omw-1.4"),
#         ("corpora/words", "words"),
#     ]
...
#             except Exception:
#                 pass  # keep going; we’ll have runtime fallbacks

# _ensure_nltk_resources()

# # Safe wrappers so missing data never crashes the run
...
#         for p in ("punkt",):
#             try:
#                 nltk.data.find("tokenizers/" + p)
#             except LookupError:
#                 nltk.download(p, quiet=True)
#         try:
#             return _wt(text)
#         except Exception:
#             # final fallback: simple regex word splitter
#             return re.findall(r"[A-Za-z]+(?:'[A-Za-z]+)?", text)

# def safe_pos_tag(tokens, lang="eng"):
#     try:
#         return _pt(tokens, lang=lang)
#     except LookupError:
#         for pkg in ("averaged_perceptron_tagger_eng", "averaged_perceptron_tagger"):
#             try:
#                 nltk.data.find("taggers/" + pkg)
#             except LookupError:
#                 nltk.download(pkg, quiet=True)
#         try:
#             return _pt(tokens, lang=lang)
#         except Exception:
#             # last resort: mark everything as a noun
#             return [(t, "NN") for t in tokens]
# # === end bootstrap ===# put this at the very top of analyze_sentiment.py
# import nltk, os

# def _ensure_nltk():
#     bundles = [
#         ("tokenizers/punkt", "punkt"),
#         ("tokenizers/punkt_tab", "punkt_tab"),  # some NLTK builds require this
#         ("corpora/stopwords", "stopwords"),
#         ("corpora/wordnet", "wordnet"),
...
#     "unremarkable": 0, "mundane": 0, "routine": 0, "typical": 0, "uncomplicated": 1, "conventional": 1, "tolerable": 1,
#      "generic": 0,
...
# analyze_sentiment.py — cross-platform NLTK + stable paths
# analyze_sentiment.py — cross-platform NLTK + stable paths (clean header)

...
def write_cleaned_tweets(rows):
    ...
    SET c.tokens                 = row.tokens,
        ...
        c.score                  = row.score,
        // arrays are valid property types in Neo4j
        c.nonScoredWords         = row.non_scored_words,
        c.nonScoredCounts        = row.non_scored_counts,
        // JSON string if you prefer a single field for export/debug
        c.nonScoredWordFreqJson  = row.non_scored_word_freq_json
...
while True:
    ...
    for row in batch:
        ...
        # === NEW SECTION: convert dict → arrays + JSON for Neo4j ===
        non_scored_words  = list(non_scored_word_freq.keys())
        non_scored_counts = [non_scored_word_freq[w] for w in non_scored_words]
        non_scored_word_freq_json = json.dumps(non_scored_word_freq, ensure_ascii=False)
        # ==========================================================
        ...
        prepared.append({
            ...
            "label": label,
            "score": score,
            "non_scored_words": non_scored_words,
            "non_scored_counts": non_scored_counts,
            "non_scored_word_freq_json": non_scored_word_freq_json,
        })

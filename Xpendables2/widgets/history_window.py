...
class HistoryWindow(tk.Toplevel):
    def __init__(self, master, driver):
        ...
        try:
            ...
            pass  # fallback to whatever is available

        # Global dark styles (affects ttk widgets app-wide)
        ...
        ttk.Entry(filt, textvariable=self.keyword_var, width=18)\
        ...
        ttk.Entry(filt, textvariable=self.user_var, width=18)\
            .grid(row=0, column=5, padx=6)

        L(1, 0, "Date From (YYYY-MM-DD):")
        self.date_from_var = tk.StringVar()
        ttk.Entry(filt, textvariable=self.date_from_var, width=18)\
            .grid(row=1, column=1, padx=6)

        L(1, 2, "Date To (YYYY-MM-DD):")
        self.date_to_var = tk.StringVar()
        ttk.Entry(filt, textvariable=self.date_to_var, width=18)\
        ...
        ttk.Entry(filt, textvariable=self.min_score_var, width=10)\
            .grid(row=2, column=3, padx=6, sticky="w")

        L(2, 4, "Max Score:")
        self.max_score_var = tk.StringVar()
        ttk.Entry(filt, textvariable=self.max_score_var, width=10)\
            .grid(row=2, column=5, padx=6, sticky="w")
    ...
    def _build_params(self):
        ...
        try:
            if self.min_score_var.get().strip():
                p["min_score"] = float(self.min_score_var.get())
            if self.max_score_var.get().strip():
                p["max_score"] = float(self.max_score_var.get())
        except ValueError:
    ...
    def search(self):
        ...
        total = self._query_count(p)
        rows = self._query_page(p)
        start = (p["skip"] + 1) if total > 0 else 0
        end = min(p["skip"] + self.limit, total)
    ...
    def reset_filters(self):
        for var in [
            self.keyword_var, self.run_var, self.user_var,
            self.date_from_var, self.date_to_var, self.sentiment_var,
            self.min_score_var, self.max_score_var, self.contains_var
        ]:
            var.set("")
        self.skip = 0
        self.search()
    ...
    def export_csv(self):
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            ...
            for r in rows:
                w.writerow([
                    r["created_at"], r["keyword"], r["user"], r["run_id"],

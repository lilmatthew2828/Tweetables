...
class SentimentAnalysisApp:
    def __init__(self, master):
        ...
        tk.Label(self.frame, text="Xpendables: Movie Sentiment Analysis Tool",
                 font=("Helvetica", 16, "bold"), bg="#000000").pack(pady=10)

        keyf = tk.Frame(self.frame, bg="#000000"); keyf.pack(pady=5)
        tk.Label(keyf, text="Enter Movie Keyword:", font=("Arial", 12), bg="#000000").pack(side=tk.LEFT, padx=5)
        ...
        self.output_text = tk.Text(textf, wrap=tk.WORD, height=12, font=("Consolas", 11),
                                   background="#000000", foreground="#FFFFFF", insertbackground="#111827",
        ...
        btnf = tk.Frame(self.frame, bg="#000000"); btnf.pack(pady=10)

        tk.Button(
            btnf, text="Fetch Tweets", command=self.open_fetch_tweets,
            font=("Arial",12), bg="white", fg="black", padx=10, pady=5
        ).grid(row=0, column=0, padx=5)

        tk.Button(
            btnf, text="Analyze Sentiment", command=self.open_sentiment_analysis,
            font=("Arial",12), bg="white", fg="black", padx=10, pady=5
        ).grid(row=0, column=1, padx=5)

        tk.Button(
            btnf, text="Run Full Analysis", command=self.run_full_pipeline,
            font=("Arial",12), bg="white", fg="black", padx=10, pady=5
        ).grid(row=1, column=0, columnspan=2, pady=8)

        tk.Button(
            btnf, text="History", command=self.open_history_viewer,
            font=("Arial",12), bg="white", fg="black", padx=10, pady=5
        ).grid(row=2, column=0, pady=8)

        tk.Button(
            btnf, text="Admin", command=self.open_admin_screen,
            font=("Arial",12), bg="white", fg="black", padx=10, pady=5
